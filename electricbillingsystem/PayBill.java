...
public class PayBill extends JFrame implements ActionListener{
    ...
    PayBill(String meter) {
        ...
        lblmeternumber.setBounds(35,80,200,30);
        add (lblmeternumber);
        
        JLabel meternumber = new JLabel("");
        meternumber.setBounds(145,80,200,30);
        add (meternumber);
    ...
    public void actionPerformed(ActionEvent ae) {
        if(ae.getSource() == pay) {
            try {
                Conn c = new Conn();
                c.s.executeQuery("update bill set status = '"+meter+"' and month = '"+cmonth.getSelectedItem()+"'");
            }catch(Exception e) {
                e.printStackTrace();
            }
            setVisible(false);
